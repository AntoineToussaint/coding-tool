...
def _is_currently_active(coupon: Coupon, today: date) -> bool:
...
def list_active_coupons(today: Optional[date] = None) -> list[Coupon]:
    ...
    return [
        ...
        if c.expires_on is not None and _is_currently_active(c, today)
    ]
...
def is_coupon_currently_valid(code: str, today: Optional[date] = None) -> bool:
    ...
    if coupon.expires_on is None:
        return False
    return _is_currently_active(coupon, today)
