"""Canonical solution for c07_inline_function / large."""
...
NEW_COUPONS = '''"""Coupon definitions and order-level coupon application."""
...
def list_active_coupons(today: Optional[date] = None) -> list[Coupon]:
    """Return all registered coupons that are currently active (not expired)."""
    today = today or date.today()
    return [
        c for c in _coupons.values()
        if c.expires_on is not None and (c.active and c.expires_on >= today)
    ]


def is_coupon_currently_valid(code: str, today: Optional[date] = None) -> bool:
    """Quick predicate: does the named coupon exist and is it currently active?"""
    today = today or date.today()
    if code not in _coupons:
        return False
    coupon = _coupons[code]
    if coupon.expires_on is None:
        return False
    return coupon.active and coupon.expires_on >= today
...
def apply(workdir: Path) -> None:
    (workdir / "ecom" / "coupons.py").write_text(NEW_COUPONS, encoding="utf-8")
