"""Canonical solution for c11_type_changes / large."""
...
CONTENT = '''"""Aggregate reports over collections of orders.
...
def _order_date(order: Order) -> date | None:
...
def revenue_in_range(orders: Iterable[Order], start: date, end: date) -> float:
    if end < start:
...
def biggest_spender(orders: Iterable[Order]) -> int | None:
